...
/*   Updated: 2025/05/09 16:49:30 by hello_x          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (philosopher->philo->number_of_meals != -1 && philosopher->number_of_meals >= philosopher->philo->number_of_meals)
		return (true);
	return (false);
}
...
static bool check_philos(t_philo *philo)
{
	...
	{
		pthread_mutex_lock(&philo->philosophers[i].data_mutex);
		if (!check_full(&philo->philosophers[i]))
			all_eaten = false;
		if (check_death(&philo->philosophers[i]))
		{
			handle_death(philo, i);
			pthread_mutex_unlock(&philo->philosophers[i].data_mutex);
			return (true);
		}
		pthread_mutex_unlock(&philo->philosophers[i].data_mutex);
		i++;
